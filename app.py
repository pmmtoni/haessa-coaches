...
from production_engine import (
    generate_completion_tasks,
    get_selected_production_tasks,
)
...
def get_task_timing(task, today=None):
    """Return live workflow/timing information for a completion task."""
    today = today or datetime.now().date()
    status = (getattr(task, "status", None) or "").strip() or None
    assigned_date = getattr(task, "assigned_date", None)
    started_date = getattr(task, "started_date", None)
    expected_days = getattr(task, "expected_days", None)
    due_date = getattr(task, "due_date", None)

    if task.completed:
        completed_on = task.completed_date or today
        base = started_date or assigned_date or completed_on
        days_elapsed = max((completed_on - base).days + 1, 1)
        days_overdue = max((completed_on - due_date).days, 0) if due_date else 0
        return {"status": "Completed", "percent": 100, "assigned_date": assigned_date,
                "started_date": started_date, "expected_days": expected_days, "due_date": due_date,
                "days_elapsed": days_elapsed, "days_remaining": 0, "days_overdue": days_overdue,
                "is_overdue": days_overdue > 0, "day_label": "Completed"}

    if not status:
        return {"status": None, "percent": getattr(task, "percent_complete", None),
                "assigned_date": assigned_date, "started_date": started_date,
                "expected_days": expected_days, "due_date": due_date, "days_elapsed": 0,
                "days_remaining": None, "days_overdue": 0, "is_overdue": False,
                "day_label": "Not assigned"}

    base_date = started_date or assigned_date
    if not base_date:
        return {"status": status, "percent": getattr(task, "percent_complete", 0) or 0,
                "assigned_date": assigned_date, "started_date": started_date,
                "expected_days": expected_days, "due_date": due_date, "days_elapsed": 0,
                "days_remaining": expected_days, "days_overdue": 0, "is_overdue": False,
                "day_label": "Awaiting start"}

    days_elapsed = max((today - base_date).days + 1, 1)
    calculated_due = due_date or (base_date + timedelta(days=max(expected_days - 1, 0)) if expected_days else None)
    days_remaining = None; days_overdue = 0; is_overdue = False
    if calculated_due:
        delta = (calculated_due - today).days
        if delta >= 0: days_remaining = delta
        else: days_remaining = 0; days_overdue = abs(delta); is_overdue = True

    live_status = "Overdue" if is_overdue else ("In Progress" if status == "In Progress" else "Assigned")
    day_label = (f"Day {days_elapsed} of {expected_days}" if is_overdue else f"Day {min(days_elapsed, expected_days)} of {expected_days}") if expected_days else f"Day {days_elapsed}"
    percent = getattr(task, "percent_complete", None)
    if live_status == "In Progress" and expected_days:
        percent = min(round((days_elapsed / expected_days) * 100), 99)
    return {"status": live_status, "percent": percent if percent is not None else 0,
            "assigned_date": assigned_date, "started_date": started_date, "expected_days": expected_days,
            "due_date": calculated_due, "days_elapsed": days_elapsed, "days_remaining": days_remaining,
            "days_overdue": days_overdue, "is_overdue": is_overdue, "day_label": day_label}
...
def coaches_add():
    if request.method == "POST":
        ...
        # Generate coach-specific production tasks from TaskTemplate
        task_result = generate_completion_tasks(coach)

        if task_result["created"] == 0 and task_result["existing"] == 0:
            flash(
                task_result["message"],
                "warning"
            )
        ...
        log_coach_audit(
            ...
            details=(
                ...
                f"Type={coach.coach_type}, "
                f"due_date={coach.due_date}, "
                f"tasks_loaded={task_result['created']}, "
                f"component_rows={component_count}"
            )
      ...
      [key for key in request.form.keys() if key.startswith("task_")])
        ...
        for task in coach.completion_tasks:
            ...
            if checked:
                ...
                task.assigned_date = task.assigned_date or task.completed_date
                task.started_date = task.started_date or task.assigned_date
            elif was_completed:
                task.completed_date = None
                task.status = "In Progress" if task.started_date else ("Assigned" if task.assigned_date else None)
                task.percent_complete = 0 if task.status else None
...
def coach_task_workflow(coach_id, task_id):
    ...
    if action == "assign":
        task.assigned_date = parse_date(request.form.get("assigned_date")) or today
        task.expected_days = request.form.get("expected_days", type=int) or task.expected_days or (max(math.ceil(float(task.hours or 0) / 8), 1) if float(task.hours or 0) > 0 else None)
        task.due_date = (task.assigned_date + timedelta(days=max(task.expected_days - 1, 0))) if task.expected_days else task.due_date
        task.started_date = None; task.completed = False; task.completed_date = None
        task.status = "Assigned"; task.percent_complete = 0
    elif action == "start":
        task.assigned_date = task.assigned_date or today
        task.expected_days = request.form.get("expected_days", type=int) or task.expected_days or (max(math.ceil(float(task.hours or 0) / 8), 1) if float(task.hours or 0) > 0 else None)
        task.started_date = task.started_date or today
        task.due_date = (task.started_date + timedelta(days=max(task.expected_days - 1, 0))) if task.expected_days else task.due_date
        task.completed = False; task.completed_date = None; task.status = "In Progress"
        task.percent_complete = max(getattr(task, "percent_complete", 0) or 0, 1)
    elif action == "record_delay":
        delay_reason = (request.form.get("delay_reason") or "").strip()
        if not delay_reason:
            flash("Please enter a reason for the delay / exception.", "danger")
            if return_to == "production":
                return redirect(url_for("production_tasks", coach_id=coach.id))
            return redirect(url_for("coaches_edit", id=coach.id))

        # Keep a chronological history in the existing delay_notes field.
        # This avoids a schema change while allowing multiple delay events.
        timestamp = datetime.now().strftime("%d %b %Y %H:%M")
        entry = f"[{timestamp}] {delay_reason}"
        existing = (getattr(task, "delay_notes", None) or "").strip()
        task.delay_notes = f"{existing}\n{entry}" if existing else entry

        # Recording a delay is an event only. It must NEVER complete the task.
        ...
        existing_delay_notes = (getattr(task, "delay_notes", None) or "").strip()
        # If the task is overdue, at least one delay/exception event must have
        # been recorded before it can be completed.
        if task.due_date and today > task.due_date and not existing_delay_notes:
            flash("Record at least one Delay / Exception reason before completing this overdue task.", "danger")
            if return_to == "production":
                return redirect(url_for("production_tasks", coach_id=coach.id))
            return redirect(url_for("coaches_edit", id=coach.id))
        ...
        task.completed = False; task.completed_date = None; task.status = None; task.percent_complete = None
        task.assigned_date = None; task.started_date = None; task.due_date = None
    else:
...
@app.route("/production-task-bottlenecks")
@login_required
def production_task_bottlenecks():
    """Task-level production bottleneck dashboard.

    Uses CompletionTask execution dates and the timestamped delay history.
    No separate hours/log table is required.
    """
    phase_filter = request.args.get("phase", "").strip()
    status_filter = request.args.get("status", "").strip()
    min_delay_events = request.args.get("min_delay_events", 0, type=int)
    ...
    for task in tasks:
        ...
        started = task.started_date
        due = task.due_date
        completed = task.completed_date

        # Inclusive production duration: 08 Aug -> 09 Aug = 2 days.
        if assigned:
            end_date = completed or today
            actual_days = max((end_date - assigned).days + 1, 1)
        else:
            actual_days = 0

        expected_days = task.expected_days or 0
        variance = (
            actual_days - expected_days
            if assigned and expected_days
            else 0
        )

        if due and not task.completed:
            overdue_days = max((today - due).days, 0)
        elif due and completed:
            overdue_days = max((completed - due).days, 0)
        else:
            overdue_days = 0

        delay_text = (getattr(task, "delay_notes", None) or "").strip()
        delay_events = [line.strip() for line in delay_text.splitlines() if line.strip()]

        if task.completed:
            status = "Completed"
        elif (task.status or "").strip() == "In Progress":
            status = "In Progress"
        elif assigned:
            status = "Assigned"
        else:
        ...
        if overdue_days > 0:
            risk = "High" if overdue_days >= 3 or len(delay_events) >= 3 else "Medium"
        elif variance > 1:
            risk = "Medium"
        elif variance > 0:
            risk = "Watch"
        ...
        activity_rows.append({
            ...
            "coach_no": task.coach_no,
            "phase": task.phase or "Unspecified",
            "section": task.section or "Unspecified",
            "task": task.task,
            "status": status,
            "assigned": assigned,
            "started": started,
            ...
            "delay_history": delay_events,
            "risk": risk,
        })

    filtered = [
        row for row in activity_rows
        if (not phase_filter or row["phase"] == phase_filter)
        and (not status_filter or row["status"] == status_filter)
        and row["delay_events"] >= min_delay_events
    ]

    # Aggregate by production activity (phase + section + task).
    grouped = {}
    for row in filtered:
        key = (row["phase"], row["section"], row["task"])
        ...
        item["count"] += 1
        item["active_count"] += 1 if row["status"] in ("Assigned", "In Progress") else 0
        item["completed_count"] += 1 if row["status"] == "Completed" else 0
        item["overdue_count"] += 1 if row["overdue_days"] > 0 else 0
        item["delay_events"] += row["delay_events"]
        item["max_overdue"] = max(item["max_overdue"], row["overdue_days"])
        item["coaches"].add(row["coach_no"])
        item["rows"].append(row)

        if row["assigned"] and row["expected_days"]:
            item["variance_total"] += row["variance"]
            item["variance_samples"] += 1

    bottlenecks = []
    for item in grouped.values():
        avg_variance = (
            round(item["variance_total"] / item["variance_samples"], 1)
            if item["variance_samples"] else 0
        )

        if item["max_overdue"] >= 3 or item["delay_events"] >= 3:
            risk = "High"
        elif item["overdue_count"] > 0 or avg_variance > 1:
            risk = "Medium"
        elif avg_variance > 0 or item["delay_events"] > 0:
            risk = "Watch"
        else:
            risk = "Low"
        ...
        bottlenecks.append({
            ...
            "delay_events": item["delay_events"],
            "avg_variance": avg_variance,
            "max_overdue": item["max_overdue"],
            "coach_count": len(item["coaches"]),
            "risk": risk,
        })

    bottlenecks.sort(
        key=lambda row: (
            {"High": 3, "Medium": 2, "Watch": 1, "Low": 0}[row["risk"]],
            row["max_overdue"],
            row["delay_events"],
            row["avg_variance"],
        ),
    ...
    phases = sorted({row["phase"] for row in activity_rows})
    statuses = ["Not started", "Assigned", "In Progress", "Completed"]

    summary = {
        "total_tasks": len(activity_rows),
        "active_tasks": sum(
            1 for row in activity_rows
            if row["status"] in ("Assigned", "In Progress")
        ),
        "completed_tasks": sum(
            1 for row in activity_rows if row["status"] == "Completed"
        ),
        "overdue_tasks": sum(
            1 for row in activity_rows if row["overdue_days"] > 0
        ),
        "delay_events": sum(row["delay_events"] for row in activity_rows),
        "high_risk": sum(1 for row in bottlenecks if row["risk"] == "High"),
    }

    return render_template(
        "production_task_bottlenecks.html",
        bottlenecks=bottlenecks,
        activity_rows=filtered,
        phases=phases,
        statuses=statuses,
        phase_filter=phase_filter,
        status_filter=status_filter,
        min_delay_events=min_delay_events,
        summary=summary,
    )
...
def delivery_graph():
    ...
    capacity_baseline = (
        ...
        if remaining_months and incomplete_count
        else 0
    )
...
def export_database_csv():
    ...
    writer.writerow([
        ...
        "status", "percent_complete", "assigned_date", "started_date", "expected_days", "due_date",
    ...
    for t in CompletionTask.query.order_by(CompletionTask.coach_no.asc(), CompletionTask.phase.asc(), CompletionTask.section.asc()).all():
        writer.writerow([
            ...
            getattr(t, "percent_complete", None),
            getattr(t, "assigned_date", None),
            getattr(t, "started_date", None),
            getattr(t, "expected_days", None),
...
#     tasks = (
#         ProductionWorkLog.query
#         .filter_by(coach_id=coach.id)
#         .order_by(
#             ProductionWorkLog.work_date.desc(),
#             ProductionWorkLog.created_at.desc()
#         )
#         .all()
#     )

#     # Outstanding Completion Tasks only
#     available_tasks = (
#         CompletionTask.query
#         .filter_by(
#             coach_id=coach.id,
...
#         )
#         .order_by(
#             CompletionTask.phase,
#             CompletionTask.section,
#             CompletionTask.task
#         )
#         .all()
#     )
...
#     outstanding_tasks = len(available_tasks)

#     total_hours = (
#         db.session.query(
#             db.func.coalesce(
#                 db.func.sum(ProductionWorkLog.hours),
#                 0
#             )
#         )
#         .filter(
#             ProductionWorkLog.coach_id == coach.id
#         )
#         .scalar()
#     )
...
@login_required
def production_tasks(coach_id):
    """Daily production execution and monitoring screen."""
    coach = Coach.query.get_or_404(coach_id)
    ...
    task_timing = {task.id: get_task_timing(task) for task in production_tasks}

    completed_count = sum(1 for task in production_tasks if task.completed)
    in_progress_count = sum(
        1 for task in production_tasks
        if (getattr(task, "status", None) or "").strip() == "In Progress"
    )
    assigned_count = sum(
        1 for task in production_tasks
        if (getattr(task, "status", None) or "").strip() == "Assigned"
    )
    overdue_count = sum(
        1 for task in production_tasks
        if task_timing[task.id]["is_overdue"] and not task.completed
    )
    total_duration = sum(
        task_timing[task.id]["days_elapsed"]
...
@login_required
def production_tasks_dashboard():
    ...
    return render_template(
        ...
        "production_tasks_dashboard.html",  
